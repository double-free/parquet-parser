...
auto readColumnValues(
    parquet::TypedColumnReader<ParquetDataType> &typedColReader, int64_t len) {
  ...
  while (typedColReader.HasNext() && totalReadCount < len) {
    ...
    // readCount == 0 means reaching the end of page (but not necessarily the
    // column) set it as 1 to enter the loop, its value will be override in
    // ReadBatch
    for (int64_t readCount = 1; readCount > 0; totalReadCount += readCount) {
      typedColReader.ReadBatch(len - totalReadCount,
                               definition_levels.data() + totalReadCount,
                               repetition_levels.data() + totalReadCount,
                               result.data() + totalReadCount, &readCount);
      // std::cout << "Parsing parquet column [" <<
      // typedColReader.descr()->name()
      //           << "] expect to read [" << len << "], start from [" <<
      //           totalReadCount
...
public:
  ...
  template <typename DType>
  std::vector<typename DType::c_type> getColumnBatch(std::string colName,
                                                     int64_t batchSize) {
    ...
    if (readCount < batchSize) {
      // data tail
      // std::cout << "[ParquetParser] expected to read " << batchSize << ",
      // actually read "
      //           << readCount << "\n";
    }
...
template <typename T> class CachedParquetParser {
public:
